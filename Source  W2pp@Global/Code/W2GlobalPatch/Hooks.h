...
public:
    ...
    {
        DWORD p = 0;
        if (VirtualProtect(reinterpret_cast<LPVOID>(dwAddress), dwSize, PAGE_EXECUTE_READWRITE, &p) == 0)
            return;

        for (unsigned int i = 0; i < dwSize; i++)
            *(unsigned char*)(dwAddress + i) = 0x90;

        if (VirtualProtect(reinterpret_cast<LPVOID>(dwAddress), dwSize, p, &p) == 0)
            return;
    }
    ...
    void setHookFunction(const eHookType hType, unsigned int hSourceAddress, void *func, unsigned int hNopSize = 0)
    {
        DWORD protect = 0;
        ...
        auto distance = ((reinterpret_cast<unsigned int>(func) - hSourceAddress) - instructionSize);
    ...
    void setArrayValue(unsigned int *hSourceAddress, I *hValue,int count)
    {
        ...
        {
            DWORD protect = 0;
            if (VirtualProtect(reinterpret_cast<LPVOID>(hSourceAddress[i]), sizeof(I), PAGE_EXECUTE_READWRITE, &protect) == 0)
                continue;

            *(I*)hSourceAddress[i] = hValue[i];

            if (VirtualProtect(reinterpret_cast<LPVOID>(hSourceAddress[i]), sizeof(I), protect, &protect) == 0)
                continue;
        }
