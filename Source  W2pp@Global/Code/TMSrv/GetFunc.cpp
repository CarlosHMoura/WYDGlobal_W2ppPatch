...
int GetCreateMob(int mob, MSG_CreateMob *sm)
{
	...
	if (mob < MAX_USER)
	{
		...
		sm->MortalFace = pMob[mob].extra.MortalFace;
	}
	...
	if (mob >= MAX_USER)
	{
	    sm->Score.Ac = pMob[mob].MOB.Clan == 4 ? 0 : 1;
		sm->Score.Con = (short)sm->Score.Con < 0 ? 0 : sm->Score.Con;
	}
	...
	for (int i = 0; i < MAX_EQUIP; i++)
	{
		...
		sm->RefValue[i] = item->getSancValue();

		if (mob < MAX_USER && pUser[mob].costumehide)
		{
			if (i == 12)
			{
