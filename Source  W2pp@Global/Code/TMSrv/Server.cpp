...
char FailAccount[MAX_USER][16];

CPSock ListenSocket;

char DBServerAddress[32] = { 0, };
int DBServerPort = 7514;
...
void DrawConfig(int wb)
{
	...
	sprintf(String, "potioncount %d   partybonus %d   guildboard %d",
		 POTIONCOUNT, PARTYBONUS, GUILDBOARD);
...
int  IsFree(STRUCT_SELCHAR *sel)
{
	...
	{
		if (sel->Name[i][0] != 0 && sel->Score[i].Level >= 35 && sel->Score[i].Level < 999)
			return 1;
	}
...
int GenerateSummon(int conn, int SummonID, STRUCT_ITEM *sItem, int Num)
{
	...
	for (int i = SummonNum; i < Num; i++)
	{
		...
		if (sItem->sIndex)
		{
			int mountsanc = sItem->stEffect[1].cEffect;
			if (mountsanc >= 100)
				mountsanc = 100;
			...
			if (face >= 315 && face < 345)
			{
				...
				int difcon = mcon - mUnk;
				con = mountsanc * (mcon - mUnk) / 100 + mUnk;
			}
...
BOOL WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	CreateConsole("TMServer 7.64 ( W2GlobalPatch ) W2pp@SeiTbNao");
	...
	sscanf(name, "%hhd.%hhd.%hhd.%hhd", &LocalIP[0], &LocalIP[1], &LocalIP[2], &LocalIP[3]);
...
void RegenMob(int conn)
{
	if (conn > 0 && conn < MAX_USER)
	{
		...
		pUser[conn].RegenTime++;

		if (!(pUser[conn].RegenTime % 450))
		{
...
void DoRecall(int conn)
{
	...
	if(pMob[conn].extra.ClassMaster == MORTAL && pMob[conn].MOB.CurrentScore.Level < 35)
	{
		x = 2112 + rand()%5 - 3;
		y = 2042 + rand()%5 - 3;
	}
