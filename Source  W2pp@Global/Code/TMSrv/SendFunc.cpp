...
void SendEquip(int conn, int skip)
{
	...
	for (int i = 0; i < MAX_EQUIP; i++)
	{
		...
		sm_ue.RefValue[i] = item->getSancValue();
		if (pUser[conn].mountcostumehide)
		{
			sm_ue.HideCostume = pUser[conn].mountcostumehide;
			sm_ue.Unk[0] = 1;
			sm_ue.Unk[1] = 1;
			sm_ue.Unk[2] = 1;
		}
		if (pUser[conn].costumehide)
		{
			if (i == 12)
			{
				sm_ue.HideCostume = pUser[conn].costumehide;
				sm_ue.Unk[0] = 1;
				sm_ue.Unk[1] = 1;
				sm_ue.Unk[2] = 1;
				sm_ue.Equip[i] = 0;
...
void SendScore(int conn)
{
	...
	sm_vus.Resistencia.Fogo = pMob[conn].MOB.Resistencia.Fogo;
	sm_vus.Resistencia.Gelo = pMob[conn].MOB.Resistencia.Gelo;
	sm_vus.Resistencia.Sagrado = pMob[conn].MOB.Resistencia.Sagrado;
...
void SendAffect(int conn)
{
	...
	{
		if (pMob[conn].Affect[i].Type && pMob[conn].Affect[i].Time >= 32000000)
		{
			time_t now;
			...
			if ((pUser[conn].PremiumAffectTime[i] - now) <= 3600)
			{
			...
			sm.Affect[i].Value = pMob[conn].Affect[i].Value;
			sm.Affect[i].Level = pMob[conn].Affect[i].Level;
			sm.Affect[i].Time = (int)(((pUser[conn].PremiumAffectTime[i] - now) / 60 / 60 / 24 * AFFECT_1D) - 1);
		}
